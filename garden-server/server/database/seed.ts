...
// Demo user password: 123456 (bcrypt hash)
const DEMO_PASSWORD_HASH = '$2a$10$8K1p/a0dL1LXMIqHNfbXfeJ0pPqR0NtN9RgVwgAUqQ7P5m3vGFUi'

function syncUsers(db: Database.Database): void {
  const existing = db.prepare('SELECT id FROM users WHERE username = ?').get('demo')
  if (existing) return
  const hash = bcrypt.hashSync('123456', 10)
  db.prepare('INSERT INTO users (id, username, password_hash, created_at) VALUES (?, ?, ?, ?)').run(
    'user-demo', 'demo', hash, new Date().toISOString()
  )
}
